...
public class Boid : MonoBehaviour {
	...
	void Update () {
		...
		rb.velocity += Cohesion() * Time.deltaTime + Separation() * Time.deltaTime + Alignment() * Time.deltaTime;
	...
	Vector3 Cohesion() {
		...
		for (int i = 0; i < flock.Boids.Length; i++) {
			if (gameObject != flock.Boids [i].gameObject) {
				totalXPos += flock.Boids [i].rb.transform.position.x;
				totalZPos += flock.Boids [i].rb.transform.position.z;
			}
		}
